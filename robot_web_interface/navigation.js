...
class MapRenderer {
    ...
    screenToWorld(px, py) {
        ...
        // Convert screen pixel to map cell coordinates
        const cellX = (px - this.panX) / this.zoom;
        const cellY = (py - this.panY) / this.zoom;

        // Convert map cell to world coordinates
        // Map origin is at bottom-left, but canvas (0,0) is top-left
        const worldX = cellX * res + info.origin.position.x;
        const worldY = (info.height - cellY) * res + info.origin.position.y;
    ...
    worldToScreen(wx, wy) {
        ...
        const cellX = (wx - info.origin.position.x) / res;
        const cellY = info.height - (wy - info.origin.position.y) / res;

        // Convert map cell to screen pixel coordinates
        const screenX = cellX * this.zoom + this.panX;
        const screenY = cellY * this.zoom + this.panY;
    ...
    showOrientationSelector(clickX, clickY) {
        ...
        overlay.addEventListener('click', (e) => {
            ...
            // Note: canvas Y is inverted, so negate dy for correct orientation
    ...
    publishGoal() {
        ...
        // Convert orientation to quaternion (rotation around Z-axis)
        const qz = Math.sin(this.goalOrientation / 2);
        const qw = Math.cos(this.goalOrientation / 2);

        const msg = {
            header: { 
                frame_id: "map",  // Goals should be in map frame
        ...
        console.log("Goal position (x, y):", this.goalPos.x.toFixed(2), this.goalPos.y.toFixed(2));
        console.log("Goal orientation (degrees):", (this.goalOrientation * 180 / Math.PI).toFixed(1));
    ...
    renderMap() {
        ...
        if (this.robotPos) {
            ...
            const ry = h - (this.robotPos.y - info.origin.position.y) / info.resolution;
            ...
            if (this.robotOri) {
                ...
                this.ctx.lineTo(
                    ...
                    ry - Math.sin(yaw) * (10 / this.zoom)  // Negate Y for canvas coordinates
                );
                this.ctx.stroke();
            }
        ...
        if (this.goalPos) {
            ...
            const gy = h - (this.goalPos.y - info.origin.position.y) / info.resolution;
            ...
            this.ctx.lineTo(
                ...
                gy - Math.sin(this.goalOrientation) * arrowLen  // Negate Y for canvas coordinates
            );
            this.ctx.stroke();
